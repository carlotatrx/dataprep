...
write_flags_f <- function (infile, qcfile, outpath, note = "", match = TRUE) {
  ...
  if (file.exists(qcfile)) {
    ...
    if (nrow(flags) == 0) {         # for no qc  found
      warning("QC file is empty — no flags to apply. SEF will still be written with QC metadata.")
    } else {
      ...
      if (flags$Var[1] != Data$Var[1]) {
        warning(paste("Variable mismatch between flags and data in file:", infile, "\nSkipping this file."))
        return(invisible(NULL))  # Exit the function cleanly without stopping the loop
      }
      ...
      if (length(i) > 0) {
        ...
        warning("No flags matched, but SEF file will still be written with QC metadata.")
      }
    } 
    
  } else {
    warning("QC file not found. Proceeding to write SEF file with QC metadata only.")
  }
