...
lat <- 54.581
lon <- -5.939
alt <- 18 # Elevation of Queen's College campus (~60 ft)
metaHead <- "Observer=Prof. Thomas Andrews | Location=Queen's College, Belfast | DataSource=RIA Manuscripts 12M16 & 24 O 39/JOD/3"

raw <- read.csv(paste0(indir, name, "/Belfast_1851-1852.csv"),
...
df <- raw %>%
  mutate(
    hour = NA,
    minute = NA,
    # Standardizing to the 9 a.m. RIA guideline
    obs_time_label = "9a.m.",
    meta.Tx = case_when(
      !is.na(maxF) ~ paste0("orig=", maxF, "F | obs.time=", obs_time_label, " | Note=Instrument quality questioned by observer"),
      is.na(maxF)  ~ paste0("obs.time=", obs_time_label)
    ),
    meta.Tn = case_when(
      !is.na(minF) ~ paste0("orig=", minF, "F | obs.time=", obs_time_label, " | Note=Instrument quality questioned by observer"),
