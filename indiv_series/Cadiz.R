...
raw <- read.csv("/scratch3/PALAEO-RA/daily_data/original/Cadiz/CAD_SFP.csv", na=c("","NA"))
...
write_sef_f(
  ...
  period    = "day",
  units   = units(var),
  metaHead = "PTC=Y | PGC=N | merged from Cadiz, Huelva | homogenized | QC=Y",
)



# Cadiz from Mariano Barriendos -------------------------------------------
